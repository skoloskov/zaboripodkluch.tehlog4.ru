	...
	foreach ($arResult['ITEMS'] as $key=>$item)
              ...
              <div class="company-content__left">
                ...
                <div class="company-content__desc">
                  <p class="company__description">
					<?=$start?><?/*if (strlen($end)>0){
						?><span
                      data-target="<?=$key?>"
                      class="text__visible invisibled"
                    ><?=$end;?></span
                    ><a data-path="<?=$key?>" class="company__link">>>></a>
					<?}*/?>
		...
		<?}?>
                ...
                <span class="company__time"
                  ><strong>Удобное время для связи:</strong> <?=$item['PROPERTIES']['WORK_TIME']['VALUE']!='' ? $item['PROPERTIES']['WORK_TIME']['VALUE'] : 'с 10.00 до 19.00'?></span
                >
				<?if (isset($item['PROPERTIES']['OBJECTS']) && count($item['PROPERTIES']['OBJECTS'])>0){
					?>
					...
					foreach ($item['PROPERTIES']['OBJECTS'] as $object){
						...
						<div class="slider__item">
							...
							<span class="slide__desc"><?=$object['NAME']?></span>
							<span class="slide__price"><?=$object['PROPERTIES']['PRICE']['VALUE']?></span>
						</div>
                ...
                <div class="company__slider">
                  <div class="slider__item">
                    <img class="slide__img" src="<?=SITE_TEMPLATE_PATH;?>/img/slide_img1.jpg" alt="" />
                    <span class="slide__desc">Монтаж кровельного ...</span>
                    <span class="slide__price">17 000</span>
                  </div>
                  <div class="slider__item">
                    <img class="slide__img" src="<?=SITE_TEMPLATE_PATH;?>/img/slide_img2.jpg" alt="" />
                    <span class="slide__desc">Пристройка сентябрь 2020</span>
                    <span class="slide__price">125 000</span>
                  </div>
                  <div class="slider__item">
                    <img class="slide__img" src="<?=SITE_TEMPLATE_PATH;?>/img/slide_img1.jpg" alt="" />
                    <span class="slide__desc">Монтаж кровельного ...</span>
              ...
              <div class="company-content__right">
				<?if ($arResult['MAX_SHOW_COUNTER']>0){
					...
					<?if (count($item['PROPERTIES']['REVIEWS'])>0){?>
                ...
                <div class="company-content__work">
                  <div class="security__block">
                    <span class="security__el security__contract"
                      >Работает по договору</span
                    >
                    <span class="security__el security__guarantee"
                      >Дает гарантию</span
                    >
                  </div>
                  <div class="team">
                    <span class="team__block"
                      >Свободных бригад:
                ...
                <!-- Слайдер Отзывы -->
				<?if (isset($item['PROPERTIES']['REVIEWS']) && count($item['PROPERTIES']['REVIEWS'])>0){
					?>
					<div class="company-comment__slider">
					<?foreach ($item['PROPERTIES']['REVIEWS'] as $review){
						?>
						<div class="slider__item comment-content">
							<div class="comment-content__header">
							  <img
								class="comment-content__img"
								src="<?=SITE_TEMPLATE_PATH;?>/img/user_img.png"
								alt=""
							  />
							  <div class="comment-content__block">
								<div class="comment-content__title"><?=$review['NAME']?></div>
								<div class="comment-content__range">
									<div class="rating" data-value="<?=(int)$review['PROPERTIES']['RATING']['VALUE']?>"></div>
								  <?/*<span class="range__star"></span>
								  <span class="range__star"></span>
								  <span class="range__star"></span>
								  <span class="range__star"></span>
								  <span class="range__star"></span>*/?>
								  <span class="range__date"><?=$review['PROPERTIES']['DATE']['VALUE']?></span>
								</div>
							  </div>
							</div>
							<div class="company-comment__comment">
							  <?=mb_substr($review['DETAIL_TEXT'],0, 100)?>...
							</div>
						  </div>
						<?
					}
                ...
                <ul class="company-comment__list">
					...
					$price_count=0;
					?>
					<?foreach ($item['PROPERTIES']['SERVICES']['VALUE'] as $key=>$value){
						?>
						<li class="company-comment__item">
							<span class="company-comment__el company-comment__desc"
							  ><?=$value?></span
							>
							<span class="company-comment__el company-comment__price"
							  ><?=$item['PROPERTIES']['SERVICES']['DESCRIPTION'][$key]?> руб.</span
							>
						  </li>
						<?
						$price_count++;
			  ...
			  <div class="company-content__bottom">
				<a href="<?=$item['DETAIL_PAGE_URL']?>">О компании</a>
			  </div>
            </li>
			<?
